...
- [x] 5-1. メインアプリ画面のHTML/CSS作成（`app.html` + `style.css`）
  - レスポンシブ3カラムレイアウト（カレンダー / メイン / グラフ）
  - モバイル時は縦積み
- [x] 5-2. `journal.js`：日記投稿フォームの実装
  - テキストエリア（日記本文）
  - 気分スコア選択UI（1〜10のスライダー or ボタン）
  - 画像アップロードUI（任意）
  - 1日1回制限の表示制御
- [x] 5-3. `journal.js`：日記一覧・詳細表示の実装
- [x] 5-4. `journal.js`：日記編集・削除の実装
- [x] 5-5. `app.js`：アプリ初期化・認証チェック・画面切り替え

---

### STEP 6: カレンダー表示機能
- [x] 6-1. `calendar.js`：月別カレンダーのレンダリング
- [x] 6-2. 日記を書いた日を色付きでマーク表示
- [x] 6-3. 日付クリックで該当日の日記を表示
- [x] 6-4. 前月・翌月ナビゲーション
- [x] 6-5. 連続投稿日数（ストリーク）の計算・表示
- [x] 6-6. カレンダーのレスポンシブ対応

---

### STEP 7: 気分分析グラフ
- [x] 7-1. Django側：月別気分データ取得APIの作成
  - GET `/api/journals/mood-stats/?month=YYYY-MM` - 月別気分スコア一覧
  - GET `/api/journals/mood-stats/average/` - 月平均スコア
- [x] 7-2. `mood.js`：Chart.js を使った折れ線グラフの描画
  - X軸: 日付、Y軸: 気分スコア（1〜10）
- [x] 7-3. 月別切り替え機能
- [x] 7-4. 月平均スコアの表示
- [x] 7-5. グラフのレスポンシブ対応
...
### STEP 8: デザイン・UI仕上げ
- [x] 8-1. アースカラー系のカラーパレット定義（CSS変数）
- [x] 8-2. フォント設定（Noto Sans JP / Rounded Mplus）
- [x] 8-3. 温かみのあるUI装飾（角丸、影、グラデーションなど）
- [x] 8-4. モバイル表示の最終調整
- [x] 8-5. ローディング・エラー表示のUI

---

### STEP 9: テスト・動作確認
- [x] 9-1. ユーザー登録 → ログイン → 日記投稿の一連のフローテスト
- [x] 9-2. Google OAuth認証のテスト
- [x] 9-3. 日記の編集・削除テスト
- [x] 9-4. カレンダー表示・日付クリックのテスト
- [x] 9-5. 気分グラフの表示テスト
- [x] 9-6. レスポンシブ表示テスト（PC / タブレット / スマホ）
- [x] 9-7. 1日1件制限の動作確認
- [x] 9-8. 画像アップロードのテスト
